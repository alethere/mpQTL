...
cof <- sample(c(1,2,3,NA),200,replace=T,prob=c(0.30,0.30,0.3,0.1))
cof <- cbind(cof,runif(200))
cof <- cbind(cof,cof,cof)

cof.type <- rep(c("categorical","numerical"),3)


res <- map.QTL(
  phenotypes = cbind(phenotypes,phenotypes,phenotypes,phenotypes), 
  genotypes = genotypes,  
  ploidy = 4,
  map = map, 
  K = NULL,
  ...
  no_cores = 6,
  cofactor = cof,
  cofactor.type = cof.type)

for(i in 1:5){
  plot(-log10(res[[i]]$pval),ylim=c(0,10))
}
