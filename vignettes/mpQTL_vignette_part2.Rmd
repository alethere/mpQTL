...
```{r gen_tables,echo=F}
rownames(snp) <- map$marker
tab <- knitr::kable(snp[1:10,1:4],format = "html",caption = "Dosages of 4 individuals")
kableExtra::kable_styling(tab,bootstrap_options = c("stripped","hover","condensed"))

tab <- knitr::kable(anc[1:10,1:4],format = "html",caption = "Haplotypes of 1 individual")
kableExtra::kable_styling(tab,bootstrap_options = c("stripped","hover","condensed"))
```
...
Unlike biparental populations, multiparental populations, breeding germplasm and diversity panels can have genetic structure. It is useful to evaluate the genetic structure in our population to understand it, check whether it follows our expectations (siblings are closely related, clones are genetically identical...). To estimate genetic structure, one can use a genetic distance matrix, also known as **kinship matrix**, $K$. 
...
```{r kinship}
Kd <- calc.K(t(snp))
Kh <- calc.K(t(anc),haplotypes = T, ploidy = 4)
...
In the heatmaps, when two individuals are more genetically similar, their cells are lighter, and since our individuals are ordered per family, this creates a checkboard-like pattern. The heatmaps also show us which families are more closer related to each other, due to sharing similar parents.

The same information can be seen in the PCoA plots, but we will not discuss them here. To know more see [the PCoA section](#pcoa) 
...
Besides using the default calculation of genetic structure through `calc.K()`, other distance matrices can be used, specifying them with the parameter `K = your_distance_matrix`.
...
```{r pcoa_visualization,fig.height=7,fig.width=9,out.width=1000}
...
```{r pcoa_interpretation,,fig.height=7,fig.width=9,out.width=1000}
...
skyplot(pv1,map,chrom = 2,main ="Skyline lot of chromosome 2",threshold = 3.95)

#The "chromosomes" can also be expressed as characters
map_letters <- map
map_letters$chromosome <- LETTERS[map_letters$chromosome+1]

skyplot(pv1,map_letters,main="Skyline plot where chromosomes are characters",chrom = c("C","E"),small = T,threshold = 3.95)
```
