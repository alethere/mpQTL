...
map <- data$map
phe <- data$pheno[,c(3,7)]
colnames(phe) <- c("phenotype_1","phenotype_2")
dos <- as.matrix(data$dosage)
hap <- as.matrix(data$founder)
sample_pv <- -log10(data$result[[4]]$pval)
sample_pv2 <- -log10(data$result[[8]]$pval)
cof <- data$cofactor
```
...
The main function of `mpQTL` is `map.QTL()`, a wrapper that helps build the required model matrices, implements parallelized computation, chooses between linear or mixed modelling, can impute missing genotype values and can calculate a significance threshold based on a permutation test. Since this wrapper has many parameters, we will introduce the different features separately.
...
pv <- -log10(result_dos$phenotype_1$pval)
...
```{r}
pv <- -log10(result_hap$phenotype_1$pval)
...
result_Q$phenotype_1$beta[1]
```

Note that the fixed effects provided for $Q$ are added after the intercept and before the genetic effect(s).

```{r}
pv <- -log10(result_Q$phenotype_1$pval)
skyplot(pv,map,main="QTL detection with linear model and Q correction")
```
...
result_Qpco$phenotype_1$beta[1]
```

In this case there are four effects: intercept, Q1, Q2 and dosage effect.

```{r}
pv <- -log10(result_Qpco$phenotype_1$pval)
skyplot(pv,map,main="QTL detection with linear model and Qpco correction")
```

If we look at the manhattan plot obtained with this method, we see that it's very similar to the one above! This reflects the usefulness of the "Qpco" method.
...
pv <- -log10(result_mix$phenotype_1$pval)
...
```{r}
result_mix$phenotype_1$beta[1:3]
```
...
without_cofactor <- -log10(result_mix$phenotype_2$pval)
with_cofactor <- -log10(result_cof$phenotype_2$pval)
...
```{r}
pv <- -log10(result_perm$phenotype_1$pval)
thr <- result_perm$phenotype_1$perm.thr
...
```{r}
pv <- -log10(result_NA$phenotype_1$pval)
...
```{r skyplot}
#Just a skyline plot
skyplot(sample_pv,map = map,main="Example Skyline plot")

#We want to focus on chromosome 2
skyplot(sample_pv,map,chrom = 2,main ="Skyline lot of chromosome 2",threshold = 3.6)

#The "chromosomes" can also be expressed as characters
map_letters <- map
map_letters$chromosome <- LETTERS[map_letters$chromosome]

skyplot(sample_pv,map_letters,main="Skyline plot where chromosomes are characters",
        chrom = c("B","D"),small = T)
...
```{r comp_skyplot}
comp.skyplot(list(sample_pv,sample_pv2),map,main = "Comparison of two p-value distributions",threshold = 4)
```
...
#This is better
pheno_box(phe[,1],gen_hap,haplotype = T,ploidy = 4, hap.select = c(57,37,69,46,23),
